...
**Prepared by:** RSK Online Services
...
Client owns everything outright: full source code, database, deployment credentials. No dependency on RSK afterward — client can hire any developer to maintain or extend it.
...
No large upfront dev cost. RSK owns, hosts, and maintains the system on a dedicated (single-tenant) VPS for this clinic. Client pays a small onboarding fee, then a monthly fee to use it — similar to renting Practo Ray, but with real-time OPD queue and full clinic branding that generic SaaS tools don't offer.
...
*Note: Meta's own business verification and per-message/conversation charges are billed by Meta directly to the client — RSK's ₹4,999 covers integration development only, not Meta's fees.*
